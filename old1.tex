...
\textbf{Setup.} We remove the gold patch and modified-file content from the LLM-with-patch judge of \S\ref{sec:cons}. The input to the predictor contains only the GitHub issue text and the affected file path—both extracted from the issue report itself (e.g., stack traces, error logs, or user-specified file references), not from the gold patch. This matches exactly the information available to a deployment-time scheduler, who sees the issue description and the file location before any fix is produced.
Our primary predictor is Qwen2.5-7B-Instruct in issue-only mode, evaluated against the Qwen2.5-7B-Instruct with-patch reference label. As a cross-model robustness check, we also evaluate Claude Sonnet 4.5 in
...
\textbf{Algorithm: the deployable approximation.} The exact priority
optimum ranks tasks by $\mathrm{cost}(x)\Delta(x)$, where
$\Delta(x)$ is the marginal benefit of extra compute. Since
$\Delta(x)$ is not available before execution and is difficult to
predict from issue text alone (Appendix~\ref{app:marginal}), we use a
deployment-time approximation that ranks tasks only by predicted
consequence $\hat{c}(x)$. Algorithm~\ref{alg:cons-route} is therefore
not the exact optimizer of Eq.~\ref{eq:budgetobj}; it is the
consequence-only routing rule used when marginal-gain estimates are
unavailable. In the fixed-quota experiments of \S\ref{sec:exp}, the
top-$q\%$ tasks by $\hat{c}$ receive the premium tier, with $q$ chosen
to match the total compute of the difficulty-aware baseline.
