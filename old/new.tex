...
\maketitle
\begin{abstract}
Adaptive test-time compute is typically formulated as a difficulty
allocation problem, where additional computation is assigned to tasks
that are harder or more likely to fail. This objective is natural for
average-accuracy benchmarks, where all errors are weighted equally, but
it ignores a key deployment property: failures have different
consequences. A documentation bug and a database migration error may
count as the same benchmark failure despite very different real-world
costs. Under a fixed compute budget, the relevant question is therefore
not only where a model is likely to fail, but where failure is costly.
We study consequence-aware test-time compute allocation, where each task
is assigned a consequence label and computation is routed according to
the expected cost of failure. A lightweight scheduler estimates
...
changes. Our experiments reveal that existing reasoning systems do not
reliably allocate computation according to error consequences. On
SWE-bench Lite, consequence is nearly orthogonal to task difficulty,
showing that high-consequence tasks are not simply the hardest tasks.
Across five contemporary reasoning models, we find limited
consequence-aware allocation, with models showing no consequence
signal, token-budget saturation, or only weak adaptation. Consequence is
predictable before solving, while the marginal value of additional
computation remains difficult to estimate. Under a matched compute
...
that adaptive inference should consider not only where models fail, but
...
Existing approaches largely answer this question through signals such as
difficulty, uncertainty, confidence, or expected accuracy improvement.
Tasks that appear harder, less certain, or more likely to benefit from
additional computation receive more resources
\citep{snell2024scaling,damani2024learning,manvi2024adaptive,
qu2026adaptivetesttime}. This formulation is natural for benchmark
evaluation, where every example contributes equally to the final score
and all failures are treated identically. However, deployment introduces
a fundamental asymmetry that average accuracy does not capture.
% {\centering
% \textbf{\textit{Not all errors carry the same cost.}}\par
% }
A coding assistant may introduce a documentation error or produce a
faulty database migration. Although both failures may count as a single
incorrect prediction in a benchmark, their real-world consequences are
fundamentally different. One may only affect presentation, while the
other can silently corrupt production data. Therefore, allocating
computation only according to failure likelihood ignores another
critical factor: the cost of being wrong.

This paper studies a different objective for adaptive inference. Under a
fixed compute budget, computation should not only target tasks where
failure is more likely, but also tasks where failure is more costly. We
introduce \emph{consequence-aware test-time compute allocation}, which
incorporates the impact of incorrect answers as an explicit factor in
compute routing. Unlike difficulty-aware allocation, which estimates
where a model is likely to fail, consequence-aware allocation estimates
where failures matter more. The goal is not to improve every task
uniformly, but to allocate limited computation toward tasks where
additional success has higher deployment value.

A key question is whether consequence provides information beyond
existing routing signals. If high-consequence tasks are simply harder,
then difficulty-aware allocation should already capture this benefit.
We show that this assumption does not hold. Across multiple consequence
labeling strategies on SWE-bench Lite, consequence is largely
independent of difficulty. High-consequence tasks appear throughout the
difficulty spectrum: some relatively easy tasks can have severe failure
impact, while some difficult tasks have limited downstream consequences.
Additional validation through human annotation, alternative rubrics,
maintainer-based severity analysis, and cost-model sensitivity further
supports this observation
(Appendices~\ref{app:iaa}, \ref{app:rubric},
\ref{app:realcost}, and~\ref{app:costmap}).

We next ask whether current reasoning models already perform
consequence-aware computation allocation implicitly. We audit
representative reasoning models by measuring how their reasoning
computation changes with task consequence. As shown in
Figure~\ref{fig:motivation}, existing systems exhibit limited
consequence-aware behavior. Some models show almost no relationship
between reasoning computation and consequence, some saturate their
available token budgets across tasks, and others exhibit only weak
adaptation. These results indicate that current reasoning mechanisms do
not automatically allocate computation according to the cost of failure
...
Motivated by this gap, we introduce a deployment-compatible scheduler
that modifies only inference-time computation allocation. The framework
contains two components. First, a lightweight predictor estimates task
consequence before solving, using only information available at
deployment time, such as the issue description and pre-solution file
references. The predictor does not access gold patches, generated
solutions, execution outcomes, or future information. Second, a scheduler
routes higher-consequence tasks to stronger compute strategies or
additional attempts under the same total budget.

The key distinction between consequence and existing routing signals is
that consequence measures the cost of failure, rather than the expected
difficulty or uncertainty of solving. While the marginal benefit of
additional computation can only be observed after execution, consequence
can be estimated before solving and used as a deployment-time routing
signal. Detailed analyses of marginal compute utility and confidence
based alternatives are provided in Appendices~\ref{app:marginal} and
\ref{app:whyfail}. Therefore, our deployable scheduler intentionally
uses consequence alone, while priority-aware variants that additionally
incorporate observed marginal gains are treated as diagnostic upper
bounds.

We evaluate consequence-aware allocation on SWE-bench Lite using
matched-budget outcomes from a 16-system compute tier. The scheduler
does not improve all tasks uniformly. Instead, it reallocates limited
computation toward tasks where failures have higher consequences. Under
the same total budget, consequence-aware routing substantially improves
high-consequence task success while keeping overall accuracy nearly
unchanged. A deployment-time predictor retains most of the gain achieved
by an oracle consequence signal, showing that the approach does not rely
on unavailable future information. Additional experiments examine
transfer settings, alternative compute mechanisms, and boundary
conditions across different task pools
(Appendices~\ref{app:crossdataset}, \ref{app:seconddomain},
\ref{app:within}, and~\ref{app:ood}).
...
\S\ref{sec:main}. If high-consequence tasks were simply the hardest
...
Appendix~\ref{app:iaa}). Appendix~\ref{app:rubric} repeats this
orthogonality check under all labeling pipelines, and
Appendix~\ref{app:subdomain} verifies that it is not driven by a
single SWE-bench sub-domain.
...
disagreement or low confidence. Appendix~\ref{app:whyfail} rules out
this interpretation. Consequence is only weakly related to
confidence-like pass-rate variance, and it does not predict where
extra compute helps. Instead, it enters the allocation objective as the
cost term: it tells the scheduler where a successful rescue would be
valuable. Appendix~\ref{app:marginal} further shows that difficulty
and confidence-like signals do not reliably recover the marginal
benefit of extra compute before execution.

\subsection{Deployment-Time Consequence Prediction}
\label{sec:pred}

A consequence-aware scheduler must decide how much computation to
allocate before a model produces a solution. Therefore, the scheduler
cannot rely on information revealed after execution, such as the gold
patch, final diff, or test outcomes. The key question is whether the
deployment cost of an incorrect answer can be estimated from
pre-solution information.

\textbf{Deployment-time inputs.}
Our predictor receives only information available before solving,
including the GitHub issue description and file references explicitly
provided in the issue report. It does not access the gold patch, the
reference modified files, generated solutions, execution outcomes, or
any information derived from the final diff. This setting matches the
information available to a real inference-time scheduler.

\textbf{Issue-only consequence predictor.}
We use a lightweight issue-only predictor to estimate
$\hat{c}(x)\in\{0,1,2\}$ under the same consequence rubric defined in
\S\ref{sec:cons}. The predictor observes only deployment-time inputs and
outputs an ordinal consequence label before any computation allocation
decision is made.
For evaluation, we compare the predicted consequence against the
LLM-with-patch reference label, which has access to the issue text,
gold patch, and modified files. This comparison measures whether
deployment-time information contains sufficient signal to recover the
post-hoc consequence assessment needed for routing.

\textbf{Consequence is predictable before solving.}
On SWE-bench Lite, the issue-only predictor achieves
Cohen's $\kappa=0.572$ against the with-patch reference label,
showing substantial agreement for a three-level ordinal consequence
classification problem. Importantly, prediction quality should be
evaluated by its impact on compute allocation rather than classification
accuracy alone. The most harmful routing failure is not an adjacent
label error, but underestimating a high-consequence task as low
consequence, which would assign insufficient computation to a task where
failure is costly.

Among the 44 tasks labeled as high consequence by the reference judge,
the issue-only predictor correctly identifies 39 tasks as high
consequence and assigns the remaining 5 to the medium category. It makes
no high-to-low mistakes ($2\rightarrow0=0/44$). This property is
particularly important for deployment because conservative
over-estimation only wastes some computation, whereas severe
under-estimation can leave costly failures insufficiently protected.
\begin{table}[t]
\centering
\caption{\textbf{Deployment-time consequence prediction on SWE-bench Lite.}
The predictor uses only pre-solution information and is evaluated
against the LLM-with-patch reference label. The key routing failure is
underestimating high-consequence tasks as low consequence.}
\label{tab:predictor}
\small
...
\end{tabular}
\end{table}
These results show that consequence is not merely a post-hoc property
available after observing the solution outcome. A substantial portion of
the signal is already present in the task description before solving,
making consequence suitable as a deployment-time routing variable.

Additional analyses evaluate cross-model predictors, confusion
patterns, calibration behavior, transfer settings, and alternative
deployment pools (Appendices~\ref{app:predictor},
\ref{app:crossdataset}, and~\ref{app:seconddomain}).
...
the scheduler improves average accuracy across all tasks, which is
generally impossible under a fixed budget. Instead, we ask whether it
can reallocate limited computation away from low-consequence tasks
toward high-consequence tasks, thereby achieving higher success rates
...
model generations and agent scaffolds, providing a realistic capability
ladder rather than a controlled single-model compute sweep. We sort the
16 systems by total resolved count and define a cheap tier from the
bottom four systems and a premium tier from the top four systems. For
each task $x$, $p_{\mathrm{cheap}}(x)$ denotes the fraction of
cheap-tier systems that solve the task, and
$p_{\mathrm{premium}}(x)$ is defined analogously for the premium tier.

All routing strategies are evaluated under the same matched compute
budget. Specifically, the top $25\%$ of tasks selected by each strategy
are routed to the premium tier, while the remaining tasks are assigned
to the cheap tier. Therefore, every strategy receives the same amount
of premium computation, and only the identity of the upgraded tasks
differs. We compare random routing, difficulty-aware routing,
consequence-aware routing, and priority-aware analysis variants.
Random routing selects a uniformly random top-$25\%$ subset and is
averaged over $1{,}000$ draws. Difficulty-aware routing selects tasks
according to difficulty, including both an outcome-derived difficulty
signal
($1-\overline{\mathrm{passrate}}$ across the 16 systems) and a
deployment-time predicted-difficulty baseline. Consequence-aware
routing selects tasks according to predicted consequence
$\hat{c}(x)$ from \S\ref{sec:pred}, with the Qwen issue-only predictor
serving as the primary deployment-time variant and the Claude issue-only
predictor providing a cross-model check. The oracle consequence variant
uses the with-patch reference label. Finally, priority-aware variants
rank tasks according to
$\hat{c}(x)\Delta(x)$ or $c(x)\Delta(x)$, where
\[
\Delta(x)=p_{\mathrm{premium}}(x)-p_{\mathrm{cheap}}(x).
\]
Because $\Delta(x)$ is computed from observed outcomes, these variants
are treated as analysis upper bounds rather than deployment-time
strategies.
...
\]
where $c(x)$ denotes the consequence label and
$p_{T(x)}(x)$ denotes the success probability under the assigned tier.
We additionally report high-consequence task success rate, which
directly reflects whether computation is shifted toward failures with
larger deployment costs. Overall accuracy is reported to distinguish
cost-aware reallocation from a uniform accuracy improvement. Bootstrap
confidence intervals, random-allocation variance, leave-one-class-2-out
checks, and split-half estimates for priority-aware variants are
reported in Appendix~\ref{app:bootstrap}. Full loss--compute Pareto
curves across premium budget fractions are provided in
...
The main result is stable under statistical, labeling, and
cross-setting checks. Bootstrap resampling, random-allocation variance,
tiebreak analysis, leave-one-class-2-out tests, and split-half
estimates for the priority-aware rows are reported in
Appendix~\ref{app:bootstrap}. The qualitative ordering is also stable
under alternative monotone cost mappings and continuous severity scores
(Appendix~\ref{app:costmap}) and under an alternative judge family with
different severity calibration (Appendix~\ref{app:judge2}). Beyond
these robustness checks, the full matched-budget allocation comparison
replicates on a disjoint database/ORM pool from SWE-bench Verified
using an independent 20-system tier: difficulty-aware routing is again
worse than random, consequence-aware routing improves over random, and
priority-aware routing is best (Appendix~\ref{app:seconddomain}).
The same ordering also holds when model identity is fixed and the only
allocated resource is the number of best-of-$k$ attempts
(Table~\ref{tab:withinmain} and Appendix~\ref{app:within}).

These results do not imply that consequence should always dominate
difficulty. Instead, the out-of-domain probes show that the
cost-weighted objective predicts when the consequence advantage should
disappear: it vanishes when costs are nearly homogeneous, no routing
signal helps when the premium tier has no headroom, and difficulty
routing wins when harder tasks are exactly where premium compute helps
(Appendix~\ref{app:ood}). Thus, the gain should be read not as a
universal advantage of consequence over difficulty, but as evidence
that the right routing signal depends on the deployment cost structure
and the marginal value of extra compute.
...
Our experiments focus on software engineering tasks, where consequence
heterogeneity is naturally observable and matched-budget evaluation is
possible. Although we validate the framework on additional software
pools and controlled within-model allocation
(Appendices~\ref{app:seconddomain} and~\ref{app:within}), broader
domains remain open. Future work could incorporate richer deployment
context, learned cost models, and adaptive policies that combine
consequence prediction with online execution feedback.
\section{Conclusion}
