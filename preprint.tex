...
700 software-engineering tasks in total. Our results reveal that consequence and difficulty are approximately orthogonal under various annotations, and that current thinking models do not allocate compute sufficiently according to consequence. Moreover, our issue-only predictor never misclassifies a high-consequence task as low-consequence across the 300 SWE-bench tasks. 
Under matched compute budgets, the fully deployment-time consequence-only scheduler reduces cost-weighted loss by $21.8\%$ relative to difficulty-aware routing and retains $93.6\%$ of the corresponding oracle gain. A priority-aware variant that additionally uses empirical marginal-gain information further reduces loss by $30.7\%$, showing the potential value of estimating where extra compute changes the outcome.
Notably, difficulty-aware routing performs worse than random, because many of the hardest tasks remain unsolved even at the premium tier and therefore have low marginal return from extra compute.
\end{abstract}
...
benchmark. These variants measure the additional value available if
marginal gain can be estimated at deployment time, but they are not
fully deployment-time schedulers as implemented here. Learning such a
marginal-gain predictor from issue text, model confidence, agent
traces, or early attempts is an important direction for future work.
...
difficulty-aware routing is again \emph{worse than random}. Because the model, prompt, and thinking budget are all held fixed and
