...
we propose consequence-aware test-time compute allocation. Instead of routing compute only by predicted difficulty, we use a lightweight predictor to estimate from the issue text how costly a task would be if solved incorrectly. The scheduler then routes higher-consequence tasks to larger compute tiers or higher thinking budgets under the same total budget. We conduct main experiments on SWE-bench Lite, evaluate cross-dataset behavior on Multi-SWE-bench mini, and replicate the full allocation comparison on a disjoint database/ORM pool from SWE-bench Verified, covering
840 software-engineering tasks in total. Our results reveal that consequence and difficulty are approximately orthogonal under various annotations, and that current thinking models do not allocate compute sufficiently according to consequence. Moreover, our issue-only predictor never misclassifies a high-consequence task as low-consequence on any of the three benchmarks. 
...
for high- vs low-consequence tasks; \S\ref{sec:audit}).
...
Qwen3-8B, near-universal saturation in Qwen3-VL-8B-Thinking, and weak
...
from SWE-bench Verified with an independent $20$-model tier system
(Appendix~\ref{app:seconddomain}).
...
replicates exactly. Evaluating genuinely different application
domains beyond software engineering (e.g., data analysis or
operations tasks) remains future work.
...
\S\ref{sec:whyfail} is domain-general:
