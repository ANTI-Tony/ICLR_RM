...
the with-patch reference, the predictor misses $0$ of $97$ true
high-consequence tasks across the four pools we study (one-sided
$95\%$ upper bound $3.0\%$; Appendix~\ref{app:predictor}).
...
consequence edge vanishes
exactly as the objective predicts (Appendix~\ref{app:ood}); and a
...
doubly-labeled tasks in the two benchmarks (and $0/97$ across all
four pools; Appendix~\ref{app:predictor}).
...
class-$2{\to}0$ cell is again empty. Across all four pools studied in this paper, the predictor has yet
to misclassify a single high-consequence task as low-consequence
relative to the with-patch reference ($0/97$;
...
share tracks the consequence-over-random margin closely (Spearman
$+0.84$ for oracle routing, $+0.66$ for the deployment-time
predictor; presented descriptively --- the ten settings share three
underlying task pools and are not independent observations, so we
attach no significance test).
...
$\kappa = 0.568$ (raw $80.0\%$) --- the third of the four pools at
$\kappa \approx 0.57$ (the base-rate-skewed Multi-SWE-bench mini
