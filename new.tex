...
Existing approaches largely answer this question through signals such as difficulty, uncertainty, confidence, or expected accuracy improvement. Tasks that appear harder, less certain, or more likely to benefit from additional computation receive more resources \citep{snell2024scaling,damani2024learning,manvi2024adaptive,qu2026adaptivetesttime}. This formulation is natural for benchmark evaluation, where every example contributes equally to the final score and all failures are treated identically. However, deployment introduces a fundamental asymmetry that average accuracy does not capture. Consider a coding assistant: it might introduce a documentation error or produce a faulty database migration. In a benchmark, both count as a single failure, but their real-world consequences could hardly be more different—one may confuse a user for a moment, while the other can silently corrupt production data, leading to weeks of recovery effort and lasting customer damage. If computation is allocated only by failure likelihood while ignoring the cost of being wrong, no amount of compute can prevent costly failures that could have been avoided. We cannot afford to ignore the cost of being wrong. 

This motivates our central object of study: consequence, defined as the deployment cost of an incorrect answer. A key question is whether consequence provides information beyond existing routing signals. If high-consequence tasks are simply harder, then difficulty-aware allocation should already capture this benefit. We first ask whether current reasoning models already allocate computation by consequence. As shown in Figure~\ref{fig:motivation}, auditing five contemporary reasoning models, we find that they do not: some show no relationship between thinking length and consequence, some saturate their token budgets, and others show only weak adaptation (Appendix~\ref{app:audit}). We then ask whether consequence is merely a proxy for difficulty. It is not: across multiple labeling strategies on SWE-bench Lite, consequence is largely independent of difficulty. High-consequence tasks appear throughout the difficulty spectrum, and this holds under human annotation, alternative rubrics, maintainer-based severity analysis, and cost-model sensitivity (Appendices~\ref{app:iaa}, \ref{app:rubric}, \ref{app:realcost}, and~\ref{app:costmap}).


\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig1_motivation.pdf}
\caption{\textbf{Current reasoning models lack consequence-aware compute
allocation.} Representative reasoning models show limited alignment
between reasoning computation and task consequence. Existing systems
exhibit three failure modes: insufficient consequence sensitivity,
computation saturation, and weak adaptation.}
\label{fig:motivation}
\end{figure}

Motivated by this gap, we study a different objective for adaptive inference. Under a fixed compute budget, computation should target not only where failure is likely, but where failure is costly. We introduce consequence-aware test-time compute allocation, which incorporates error impact as an explicit routing factor. A lightweight predictor estimates consequence from deployment-time inputs (issue description and file references), without accessing gold patches or execution outcomes; a scheduler then routes higher-consequence tasks to stronger compute under the same budget. Unlike difficulty, consequence measures failure cost rather than uncertainty, and crucially it can be estimated before solving while marginal gains cannot (Appendices~\ref{app:marginal} and~\ref{app:whyfail}). Our deployable scheduler uses consequence alone; priority-aware variants with marginal gains are diagnostic upper bounds. On SWE-bench Lite, consequence-aware routing shifts success toward high-consequence tasks without improving overall accuracy, and a deployment-time predictor retains most of the oracle gain. Additional experiments examine transfer, alternative compute, and boundary conditions (Appendices~\ref{app:crossdataset}, \ref{app:seconddomain}, \ref{app:within}, and~\ref{app:ood}).
...
Table~\ref{tab:main} shows the main result. The deployment-time Qwen consequence router reduces cost-weighted loss by $21.8\%$ relative to outcome-derived difficulty routing and by $9.3\%$ relative to random routing. Its oracle counterpart reduces loss by $23.3\%$, so the deployment-time predictor retains $93.6\%$ of the oracle-consequence gain measured from the difficulty baseline.

The more important change is visible in the class-2 success column. Under random routing, high-consequence tasks are solved at $19.4\%$. Under deployment-time predicted difficulty, they are solved at $29.0\%$. Under the deployment-time consequence router, they are solved at $51.1\%$, approaching the all-premium ceiling of $58.5\%$. Overall accuracy, however, changes little: $0.184$ for the Qwen consequence router versus $0.180$ for random. This is exactly the intended behavior. The scheduler is not making the whole benchmark easier; it is moving compute away from low-consequence tasks and toward high-consequence tasks. Average accuracy alone would hide this effect.
...
The main result is stable under statistical, labeling, and cross-setting checks. Bootstrap resampling, random-allocation variance, tiebreak analysis, leave-one-class-2-out tests, and split-half estimates for the priority-aware rows are reported in Appendix~\ref{app:bootstrap}. The qualitative ordering is also stable under alternative monotone cost mappings and continuous severity scores (Appendix~\ref{app:costmap}) and under an alternative judge family with different severity calibration (Appendix~\ref{app:judge2}). Beyond these robustness checks, the full matched-budget allocation comparison replicates on a disjoint database/ORM pool from SWE-bench Verified using an independent 20-system tier: difficulty-aware routing is again worse than random, consequence-aware routing improves over random, and priority-aware routing is best (Appendix~\ref{app:seconddomain}). The same ordering also holds when model identity is fixed and the only allocated resource is the number of best-of-$k$ attempts (Table~\ref{tab:withinmain} and Appendix~\ref{app:within}).
