...
\caption{\textbf{Native reasoning compute is not consequence-aware, despite
available headroom.}
(a) Across five reasoning models, native computation is weakly aligned
with task consequence, absent, or saturated at the token budget.
Numbers denote Spearman correlation with consequence or the fraction of
tasks hitting the cap.
(b) High-consequence tasks are not hopeless: in the 16-system
SWE-bench compute tier, class-2 success increases from $7\%$ under the
cheap tier to $59\%$ under the premium tier.}
...
incorrect answer. In this paper, consequence is represented by a coarse
ordinal label $c(x)\in\{0,1,2\}$. Low-consequence errors
($c(x)=0$) include cosmetic, formatting, documentation, or log-message
failures that may be visible but do not corrupt downstream systems.
Medium-consequence errors ($c(x)=1$) include contained functional
failures, such as wrong return values, edge-case crashes, or locally
recoverable feature errors. High-consequence errors ($c(x)=2$) include
silent data corruption, security or permission bypass, irreversible
deletion, overwriting, migration errors, or incorrect results consumed
downstream without timely visibility.
These labels are not calibrated monetary costs. They are a minimal
ordinal representation of deployment harm, chosen because they can be
applied consistently by both LLM judges and human annotators.
...
\caption{\textbf{Consequence-aware routing shifts success toward costly
failures.}
(a) Under the same premium-compute budget, class-2 success rises to
$51.1\%$ with consequence-aware routing, compared with $19.4\%$ under
random routing and $29.0\%$ under deployment-time predicted difficulty.
(b) The improvement comes from redistribution across consequence
classes rather than a uniform accuracy gain.
(c) The resulting allocation reduces cost-weighted loss; priority-aware
routing is shown as a diagnostic upper bound using empirical marginal
gain.}
\label{fig:main}
\end{figure}
...
\begin{table}[t]
\centering
\caption{\textbf{Main routing results on SWE-bench Lite.}
All strategies route the same top-$25\%$ fraction of tasks to the
premium tier. Consequence-aware routing reduces cost-weighted loss and
raises high-consequence success under the matched compute budget.
Priority-aware rows use empirical marginal gain and are analysis
variants rather than deployment-time schedulers.}
\label{tab:main}
\small
\begin{tabular}{lrrrr}
\toprule
...
\midrule
Priority-aware (Qwen consequence $\times$ marginal gain)    & $186.00$ & $+30.7\%$ & $50.6\%$ & $0.274$ \\
Priority-aware (oracle consequence $\times$ marginal gain)  & $\mathbf{179.75}$ & $\mathbf{+33.0\%}$ & $55.1\%$ & $\mathbf{0.278}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{\textbf{Controlled within-model allocation.}
Using a single fixed Claude Sonnet~4.5 agent, compute is varied only
through the number of best-of-$k$ attempts. The same strategy ordering
holds when model identity and prompt are fixed.}
...
\caption{\textbf{Consequence-aware routing is cost-structure dependent.}
(a) Routing gains vary across domains: consequence-aware routing helps
on SWE-style software tasks, while difficulty-aware routing wins on
MATH, where harder tasks also benefit more from additional reasoning.
BIRD and FinQA lie closer to the boundary regime.
(b) Consequence labels align with maintainer triage behavior in Django
Trac, where maintainer-assigned ``Bug'' rates increase from $27\%$ to
$70\%$ and $79\%$ across consequence classes.
(c) In MATH, increasing the high-to-low dollar-stakes spread can flip
the preferred routing strategy, with consequence-aware routing
overtaking difficulty-aware routing at roughly $30\times$.}
