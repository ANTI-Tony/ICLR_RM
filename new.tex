...
To fill this gap,
we propose consequence-aware test-time compute allocation. Instead of routing compute by predicted difficulty, we use a lightweight predictor to estimate from the issue text how costly a task would be if solved incorrectly, and weight each task's error by that cost in the allocation objective. The scheduler then routes higher-consequence tasks to larger compute tiers or more attempts under the same total budget. The premise is an asymmetry we verify empirically: before a task is solved, the cost of getting it wrong is far more predictable than the benefit of spending more compute on it (the latter is near-random ex ante, becoming predictable only with execution feedback). A deployer should therefore route by what is knowable in advance --- the cost of an error --- and the paper's larger message is less ``consequence is a better signal'' than a decision map for \emph{which} signal to route by, whose boundary conditions we state and then confirm out of domain.
...
Minimizing this cost-weighted loss is equivalent to maximizing the consequence-weighted success rate, and the effect is concrete: at a matched compute budget --- in a simulation over cached outcomes of $16$ public SWE-bench systems spanning capability tiers --- the fully deployment-time consequence-only scheduler raises the success rate on high-consequence tasks from $19.4\%$ (random) and $29.0\%$ (a deployment-fair predicted-difficulty baseline) to $51.1\%$, approaching the all-premium ceiling of $58.5\%$, while overall accuracy is unchanged --- the budget is simply spent where errors are costly. In loss terms this is a $21.8\%$ reduction ($17.7$--$29.9\%$ across cost mappings) over difficulty-aware routing, retaining nearly all of the oracle gain ($93.6\%$; App.~\ref{app:bootstrap}); the same ordering holds in a controlled experiment where a single fixed model allocates real best-of-$k$ attempts, isolating compute from model identity. A priority-aware variant that additionally uses empirical marginal-gain information reaches a $30.7\%$ reduction (an analysis upper bound; a split-half re-estimate shrinks the priority rows by $6$--$7$ points), showing the potential value of estimating where extra compute changes the outcome. Finally, the out-of-domain probes confirm the framework's own predictions of when the consequence edge disappears (homogeneous costs), when no signal helps (no premium headroom), and when difficulty routing prevails (positive difficulty--gain coupling on competition math).
Notably, difficulty-aware routing with outcome-derived difficulty performs worse than random --- many of the hardest tasks remain unsolved even at the premium tier --- while deployment-time predicted difficulty performs at the level of random; on both software-engineering pools, every difficulty-routing variant trails consequence routing.
\end{abstract}
...
Two of them carry most of the novelty and are worth stating up front:
(i)~before a task is solved, its downstream cost is predictable while
the marginal benefit of extra compute is not (it is near-random ex
ante, and becomes predictable only after execution feedback), which is
precisely why a deployer should route by cost; and (ii)~consequence
routing is not universally best but wins under stated, measurable
boundary conditions, which we confirm out of domain. The remaining
steps establish that consequence is a distinct, predictable signal and
that current models underuse it.

\begin{itemize}
\item[(1)] \textbf{Consequence is not difficulty, nor a disguised
confidence signal.} On $300$ SWE-bench Lite tasks ($75$ for the
human-majority study), consequence
...
\item[(4)] \textbf{Deployment-time consequence routing raises the
success rate where it matters.} Minimizing the cost-weighted loss is
equivalent to maximizing the consequence-weighted success rate, so the
right yardstick is whether consequential tasks get solved more often
--- a reallocation that does not is an empty relabeling. It does: at a
matched budget on a $16$-model SWE-bench tier, the deployment-time Qwen
consequence router lifts the success rate on high-consequence tasks
from $19.4\%$ (random) and $29.0\%$ (a deployment-fair
predicted-difficulty baseline) to $51.1\%$, near the all-premium
ceiling of $58.5\%$, with overall accuracy unchanged ($21.8\%$ lower
cost-weighted loss than difficulty routing, recovering nearly all of
the oracle gain). A controlled experiment that holds the model fixed
and varies only best-of-$k$ attempts reproduces the same ordering with
bootstrap intervals bounded away from zero under two judge families
(\S\ref{sec:withinmain}). A priority-aware
variant that additionally uses empirical marginal-gain information
reduces loss by $30.7\%$ --- but only as an analysis variant, because
that marginal gain is exactly the quantity we find unpredictable before
solving. The same analysis also explains why
...
remain unsolved at every available tier (\S\ref{sec:exp}, A priority-aware
variant that additionally uses empirical marginal-gain information
reduces loss by $30.7\%$ --- but only as an analysis variant, because
that marginal gain is exactly the quantity we find unpredictable before
solving. The same analysis also explains why
...
effort ratio the model reveals: under the $\{0,1,2\}$ mapping the
median class-$2$ task carries unboundedly more cost-weighted value than
a class-$0$ task (and twice that of class-$1$), against a revealed
class-$2$-vs-class-$0$ thinking ratio of only $1.2$:$1$.  Claude thus shows a weak emergent tendency
...
(\S\ref{sec:withinmain}). This motivates the question: how should
...
\textbf{Setup.} We remove the gold patch and modified-file content from the LLM-with-patch judge of \S\ref{sec:cons}. The input to the predictor contains the GitHub issue text and the affected file path. The file path is taken from the gold patch, so strictly speaking a deployer would first have to localize it from the issue; we therefore ablate it and find it immaterial --- removing the file path and predicting from the \emph{issue text alone} changes the predictor's agreement with the reference by $\kappa$ $=$ $-0.014$ and leaves downstream routing unchanged or slightly better (Appendix~\ref{app:predictor}), so the consequence signal is carried by the issue, not by the gold-derived file. Our primary predictor is Qwen2.5-7B-Instruct in issue-only mode, evaluated against the Qwen2.5-7B-Instruct with-patch reference label. As a cross-model robustness check, we also evaluate Claude Sonnet 4.5 in
...
Because $\sum_x \mathrm{cost}(x)\,[1-p(x)] = \sum_x \mathrm{cost}(x) -
\sum_x \mathrm{cost}(x)\,p(x)$, minimizing the cost-weighted loss is
\emph{equivalent to maximizing the consequence-weighted success rate}
$\big(\sum_x \mathrm{cost}(x)\,p(x)\big)/\sum_x \mathrm{cost}(x)$: the
goal is not to raise average accuracy but to spend the budget so that
the tasks that matter most are the ones that get solved. This is the
metric we report throughout (\S\ref{sec:exp}), because a reallocation
that does not raise the success rate on high-consequence tasks would
be an empty relabeling --- the point of routing by consequence is that
consequential tasks are solved more often.

\textbf{What the objective rewards, and its exact optimum.} Upgrading
a task $x$ from the cheap tier to the premium tier lowers the loss by
exactly $\mathrm{cost}(x)\cdot[\,p_{\mathrm{premium}}(x) -
p_{\mathrm{cheap}}(x)\,] = \mathrm{cost}(x)\cdot\Delta(x)$: the error
cost times the \emph{marginal success gain} from more compute. Under a
fixed budget with equal per-task upgrade costs, the loss-minimizing
allocation is therefore the greedy rule that upgrades tasks in
descending order of this product $\mathrm{cost}(x)\cdot\Delta(x)$ ---
routing by consequence \emph{and} marginal gain. This priority rule,
not consequence alone, is the exact optimum of
Eq.~\ref{eq:budgetobj}; it is the ``priority-aware'' strategy of
\S\ref{sec:exp}.

\textbf{Algorithm: the deployable approximation.} The priority optimum
requires $\Delta(x)$, the per-task benefit of extra compute, which is
\emph{not} predictable before a task is solved (prompted and learned
estimators of $\Delta$ are no better than chance;
Appendix~\ref{app:marginal}). We therefore deploy the
consequence-only approximation: sort tasks by the predicted
consequence $\hat{c}(x)\in\{0,1,2\}$ from \S\ref{sec:pred} in
descending order and fill compute tiers from premium to cheap until the
budget is exhausted (Algorithm~\ref{alg:cons-route}). In the
fixed-quota variants of \S\ref{sec:exp} the top-$q\%$ tasks by
$\hat{c}$ receive the premium tier, with $q$ chosen so total compute
matches the difficulty-based baseline. Routing by $\hat{c}$ alone is
exact when $\Delta$ is constant or uncorrelated with cost, and
empirically recovers most of the priority optimum's gain
(\S\ref{sec:exp}). It assumes only that $\hat{c}$ tracks the
\emph{cost-weighted} value of a correct answer; it does \emph{not}
assume that consequence predicts where compute helps ---
...
We evaluate Algorithm~\ref{alg:cons-route} with two complementary main
experiments that trade off realism against control. The first (this
section) routes tasks across a $16$-model SWE-bench
capability tier: a large-scale, deployment-flavored ladder, but one in
which ``more compute'' is confounded with model generation and
scaffold. The second (\S\ref{sec:withinmain}) removes that confound by
holding the model, prompt, and thinking budget fixed and varying only
the number of best-of-$k$ attempts --- a clean, single-lever
compute axis. The two experiments agree on every qualitative
conclusion, which is the point: the allocation advantage is not an
artifact of either setup.
...
generations. These systems differ in model generation and agent
scaffold as well as raw compute, so this tier is best read as a
realistic capability ladder rather than a controlled compute sweep ---
the controlled counterpart is \S\ref{sec:withinmain}. These include RAG baselines on Claude 2 and GPT-4,
...
\begin{table}[t]
\centering
\caption{\textbf{Main results.} Cost-weighted loss $\mathcal{L}$, its
relative reduction over the outcome-derived difficulty baseline, the
success rate on \emph{high-consequence} (class-2) tasks, and overall
accuracy on $N{=}300$ SWE-bench Lite tasks --- all at the same
top-$25\%$ premium budget (price ratio $1{:}4$). The deployment-time
Qwen consequence router more than doubles the class-2 success rate over
random ($51.1\%$ vs.\ $19.4\%$) and over a deployment-fair
predicted-difficulty baseline ($29.0\%$), approaching the all-premium
ceiling ($58.5\%$), while overall accuracy is essentially unchanged
($0.184$ vs.\ random $0.180$): the budget is spent where errors are
costly, not spread evenly, and it is the consequential tasks that get
solved. Both difficulty baselines --- outcome-derived (strongest, but
not available at deployment) and deployment-time predicted --- trail
consequence routing on the class-2 metric. The priority-aware rows use
empirical marginal gain (the exact optimum of Eq.~\ref{eq:budgetobj});
a split-half re-estimate shrinks them by $6$--$7$ points
(Appendix~\ref{app:bootstrap}). The Qwen router retains $93.6\%$ of the
consequence-oracle loss reduction.}
\label{tab:main}
\small
\begin{tabular}{lrrrr}
\toprule
...
\midrule
Difficulty-aware (outcome-derived; Snell)~\citep{snell2024scaling}   & $268.25$ & ~~$0\%$ & $6.8\%$ & $0.054$ \\
Difficulty-aware (deployment predicted)                             & $224.00$ & $+16.5\%$ & $29.0\%$ & $0.181$ \\
Random (mean over $1{,}000$ draws)                                  & $231.20$ & $+13.8\%$ & $19.4\%$ & $0.180$ \\
\midrule
Consequence-aware (Claude issue-only; cross-model)                  & $220.50$ & $+17.8\%$ & $36.9\%$ & $0.174$ \\
...
\textbf{The tasks that matter get solved more often.} The point of
reallocation is not a smaller abstract loss but a higher success rate
where it counts: the deployment-time Qwen router raises the success
rate on high-consequence (class-2) tasks from $19.4\%$ under random
routing --- and $29.0\%$ under a deployment-fair predicted-difficulty
baseline --- to $\mathbf{51.1\%}$, more than half way to the
all-premium ceiling of $58.5\%$ (Table~\ref{tab:main}, class-2 column),
at the same budget. Overall accuracy barely moves ($0.184$ vs.\ random
$0.180$), and this is by design: under a fixed budget one cannot lift
every task's success rate, so consequence routing withdraws compute
from cosmetic class-0 tasks and concentrates it on the consequential
ones. Reporting accuracy alone would hide the entire effect --- a
router that raised class-2 success from $19\%$ to $51\%$ would look
identical to random. The reallocation is therefore not an empty
relabeling: it is exactly the success-rate improvement, on the tasks
whose failures are costly, that motivates routing by consequence in
the first place. For
...
\subsection{A controlled compute lever: attempts on a single fixed model}
\label{sec:withinmain}

The $16$-model experiment answers ``does consequence-aware routing help
when a deployer picks among systems of different capability?'' but it
cannot separate compute from model identity. We therefore repeat the
allocation comparison in a setting where the \emph{only} thing that
varies is compute: a single fixed Claude Sonnet~4.5 agent (same model,
prompt, and thinking budget throughout) to which we allocate a varying
number of best-of-$k$ attempts. Attempts are a genuine compute lever
here --- best-of-$k$ pass rate rises from
$\mathrm{pass@}1 = 0.142$ to $\mathrm{pass@}8 = 0.370$ --- and the
task difficulty--gain coupling reappears with the same sign as the main
experiment ($\rho(\mathrm{difficulty}, \Delta\mathrm{success}) < 0$),
so the hardest tasks again benefit least from extra compute
(Appendix~\ref{app:within} gives the full protocol, both judges, and
the dynamic-scheduling and no-oracle extensions).

We allocate a fixed total attempt budget across the pool, upgrading
tasks to best-of-$8$ by each strategy's score and giving the rest a
single attempt; strategies differ only in \emph{which} tasks are
upgraded. The ordering exactly reproduces the cross-model result
(Table~\ref{tab:withinmain}). Under the primary (Claude) judge on
$n{=}127$ tasks, difficulty-aware routing is again \emph{worse than
random} (in $100\%$ of $1{,}000$ tiebreak draws), the fully
deployment-time issue-only consequence predictor beats difficulty by
$+18.6\%$ (task-level bootstrap $+20.2\%$, $95\%$ CI $[+10.5, +30.7]$,
positive in $100\%$ of $5{,}000$ resamples), and the priority variant
reaches $+24.5\%$ (CI $[+14.6, +35.6]$). A stricter second judge from
a different model family ($n{=}126$) preserves the ordering with
smaller but still-positive margins (consequence-predictor $+8.7\%$, CI
$[+2.0, +19.2]$; difficulty below random in $93.5\%$ of draws). With
model, prompt, and thinking budget all held fixed, the advantage
cannot be an artifact of routing across heterogeneous model tiers ---
it is a property of allocating compute by consequence.

\begin{table}[t]
\centering
\caption{\textbf{Controlled within-model allocation} ($n{=}127$
SWE-bench Lite tasks, single fixed Claude Sonnet~4.5 agent, primary
judge; compute is the number of best-of-$k$ attempts under a matched
total budget). Cost-weighted loss $\mathcal{L}$ and reduction over the
difficulty-aware baseline, with task-level bootstrap $95\%$ CIs
($5{,}000$ resamples). The full protocol, the second-judge replication,
and sequential/no-oracle extensions are in Appendix~\ref{app:within}.}
...
consequence on top of both lifts $R^2$ from $0.474$ to $0.738$. Because
the target $Y$ contains $\mathrm{cons}$ as a multiplicative factor this
gain is mechanically favored; we report it only as a non-collinearity
check (see text), not as evidence of predictive value, and therefore
do not emphasize the relative figure.}
\label{tab:r2decomp}
\small
\begin{tabular}{lc}
\toprule
...
\midrule
$\Delta R^{2}$ from adding Consequence                & $+0.264$ \;(mechanically favored; see text) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Scope of evaluation.} Our two main allocation experiments use
(i)~publicly available per-task outcomes from 16 SWE-bench Lite
leaderboard solvers and (ii)~a controlled single-model best-of-$k$
study (\S\ref{sec:withinmain}). The first lets us compare many compute
tiers on the same task set and estimate per-task marginal gains from
upgrading cheap models to premium models, but it is an offline
compute-tier study rather than a controlled intervention on a single
reasoning model. The second (\S\ref{sec:withinmain}; full protocol and
deployment-time, no-oracle extensions in Appendix~\ref{app:within})
removes that confound by holding the model, prompt, and thinking budget
fixed and varying only the number of best-of-$k$ attempts. The same qualitative ordering is reproduced: consequence-aware and
...
$21.8\%$ relative to difficulty-aware routing; a controlled experiment
that holds the model, prompt, and thinking budget fixed while varying
only best-of-$k$ attempts reproduces the same ordering
(\S\ref{sec:withinmain}). A priority-aware
...
realized compute by roughly a third. Run as a live streaming router
--- sequential arrivals, real-time budget pacing, closed-loop
verifier escalation with live inference --- the same machinery
reproduces these boundary conditions online rather than from a cache
...
the within-repository trend, which is positive throughout and, among
the backporting repositories, a significant ordered trend
($\rho = +0.175$, $p = 0.024$), though the repository-stratified CMH
contrast is underpowered ($\mathrm{OR} = 1.76$, $p = 0.14$).
...
\paragraph{A second, LLM-independent anchor: issue-tracker triage.}
Backporting is a coarse binary. A finer maintainer-assigned signal is
available for the $114$ Django tasks, which are the largest single
repository in the pool: Django triages every ticket in a Trac tracker,
recording a human-set \emph{type} (Bug / Cleanup-optimization / New
feature), a \emph{severity} (Normal / Release blocker), and process
flags --- none produced by an LLM. We retrieved this metadata for all
$114$ tickets (the ticket number is embedded in each merge-commit
subject). The maintainer-assigned type tracks the consequence label
monotonically: a task judged class-$0$ is a genuine ``Bug'' only
$26.7\%$ of the time (it is disproportionately cleanup or a new
feature), rising to $69.7\%$ at class-$1$ and $78.8\%$ at class-$2$
(ordered trend $\rho = +0.282$, $p = 0.002$; combined bug-and-blocker
severity $\rho = +0.223$, $p = 0.017$). The ``Release blocker''
severity flag, by contrast, does \emph{not} separate the classes
($13.3\% / 24.2\% / 24.2\%$, n.s.) --- unsurprisingly, since it
encodes release-cycle timing rather than deployment blast radius ---
and we report it as a null. Most usefully, Trac carries the
maintainers' own \emph{difficulty} proxy, an ``easy pickings'' flag:
it is orthogonal to the consequence label
($\rho(\mathrm{consequence}, \mathrm{easy}) = -0.12$, n.s.; at
$n{=}114$ this null has $80\%$ power only against $|\rho| \gtrsim 0.26$),
\emph{independent, non-LLM} evidence consistent with the paper's
central claim that consequence is not a relabeling of difficulty
(\S\ref{sec:cons}). Across both anchors the same conclusion holds:
the consequence labels are not merely an LLM's private construct ---
they align with what human maintainers did (backport) and how human
maintainers triaged (ticket type), while remaining distinct from how
hard those maintainers thought the tasks were.
...
\textbf{Is the file path a leak?} The predictor prompt lists the
affected file, taken from the gold patch and thus not strictly
available before the task is solved. To test whether this
gold-derived field carries the signal, we re-ran the \emph{same} model
on all $300$ tasks in two conditions --- with the file path (as used
throughout) and from the \emph{issue text alone} --- and compared each
against the with-patch reference. The file path is immaterial. On
Qwen2.5-7B-Instruct, agreement with the reference moves only from
$\kappa = 0.345$ (with file) to $\kappa = 0.331$ (issue-only), a
$-0.014$ change, with the two conditions agreeing on $83.3\%$ of tasks;
routing by the issue-only predictor is if anything \emph{better}
downstream (cost-weighted loss $217.8 \to 214.0$, class-2 success
$31.2\% \to 34.7\%$ at the top-$25\%$ budget). An independent model
(GPT-4o-mini) shows the same pattern ($\kappa$ $0.329 \to 0.287$,
downstream unchanged). (These ablation prompts are deliberately
conservative and reach lower absolute $\kappa$ than the tuned primary
predictor below; the point is the negligible \emph{difference} the file
path makes.) The consequence signal is therefore carried by the issue
report, and the deployment-time claim holds on issue text alone.
...
independent pool with an independent tier system. The difficulty-aware
row uses oracle difficulty ($1-\overline{\mathrm{passrate}}$), which is
maximally wasteful here because it targets the never-solved floor
exactly; deployment-time \emph{estimated} difficulty on this pool lands
near random ($\hat{d}$ ensemble loss $135.00$ vs random $135.61$;
Appendix~\ref{app:marginal}), so oracle difficulty is the strongest,
not a strawman, difficulty signal.
...
\midrule
Difficulty-aware (oracle difficulty)        & $154.75$ & ~~$0\%$ baseline \\
...
Catastrophic (class-$2$) outliers are rare (the Qwen judge marks only
$2\%$; a stakes-first GPT-4o judge marks all $150$ problems class-$0$),
so competition math carries little of the cost heterogeneity the
consequence signal exploits. Under the GPT-4o judge consequence is
constant across all $150$ problems, so it cannot discriminate tasks and
the priority ranking $\mathrm{cons}\times\Delta$ is governed by the
$\Delta$ term alone, which official difficulty tracks. Under the Qwen
weights, which do vary, the consequence edge over random ($+20.4\%$)
comes from concentrating premium on the few nonzero-weight tasks rather
than from tracking $\Delta$. Either
way, difficulty-aware routing \emph{wins} on MATH, beating random by
$+33.7\%$ and consequence-oracle routing by a clear margin. The framework
prescribes this in advance: on competition math, difficulty is the
correct routing signal.

\paragraph{The boundary-condition quadrant.} Across the five routed pools
...
The first main experiment (\S\ref{sec:exp}) varies compute by
routing tasks across a $16$-model tier, where ``more compute'' is
confounded with model identity. The co-main experiment of
\S\ref{sec:withinmain} removes that confound by fixing a single agent
and varying \emph{test-time compute alone}; this appendix gives its
full protocol, the choice of compute lever, both judges, and the
dynamic-scheduling and no-oracle extensions.
...
only the number of attempts is allocated, this controlled experiment
establishes that the allocation advantage is not an
artifact of routing across different model tiers: the same pattern
holds when consequence governs test-time compute within a single
fixed agent.
...
\paragraph{Judge robustness (original sample).} The table above is
graded by the Claude judge alone. To check that this does not drive the
result, we re-graded all $396$ generated attempts (across the $60$-task
pool from which the original $48$-task sample is drawn) with a judge
from a different model family (\texttt{deepseek-chat}) under the
identical judging prompt (the scaled analyses above already use both
judges throughout; this paragraph documents the attempt-level
agreement). The two
...
The evaluations elsewhere in the paper score routing policies on
\emph{cached} outcomes. This appendix closes the loop: we wrap the
full pipeline as a live streaming router and run it end-to-end with
real inference, real-time budget pacing, and a closed-loop verifier,
to check that the machinery --- and the boundary-condition framework
--- survive the move from offline scoring to online operation.

\paragraph{Setup.} A stream of $300$ tasks ($150$ FinQA
$+$ $150$ MATH, shuffled, seed $7$) arrives one at a time. Each task
carries a deployment-declared stakes tier mapped to dollars
(\$10K/\$1M/\$10M; assignment frozen,
orthogonal to difficulty by the stratified construction of
Appendix~\ref{app:ood}). Four policies process the \emph{same}
stream strictly sequentially --- every decision uses only tasks seen
so far and a real-time paced premium budget (premium calls
$\le 25\%$ of arrivals) --- with live calls to
\texttt{gpt-4o-mini} (cheap) and \texttt{gpt-4o}/\texttt{o4-mini}
(premium). \textsc{random} flips a budgeted coin; \textsc{diff}
routes on a live \texttt{gpt-4o-mini} difficulty rating against a
running top-quartile threshold; \textsc{cons} routes on the declared
stakes against the same online-threshold machinery; \textsc{seq}
takes a cheap attempt, scores it with a \emph{gold-free} verifier
(Appendix~\ref{app:marginal}; never sees the answer), and escalates
to premium only when the verifier is unsure \emph{and} stakes are
non-trivial. No routing component ever sees a gold answer; grading is
post-hoc. The run completed all $300$ tasks on all four arms with zero
errors.

\paragraph{The framework survives live operation.} The live
results reproduce the offline boundary predictions exactly
(Table~\ref{tab:online}). This stream is deliberately built from the
two \emph{non-software} regimes where the quadrant says consequence
should \emph{not} be the winning signal, and it is not: dominated by
MATH's positive difficulty--gain coupling, the live difficulty router
wins ($32.7\%$ lower realized dollar-weighted loss than random),
consequence-aware routing beats random ($18.8\%$ lower; paired bootstrap
$P = 92\%$) but not difficulty, and FinQA contributes no separation
at all (all arms within $\pm6\%$, the no-headroom regime). The
per-pool split is unambiguous --- MATH: difficulty $5{,}516$,
consequence $8{,}217$, random $13{,}013$; FinQA: all $\approx
11{,}000$ --- so the framework predicts its own online outcome, down
to which domain drives it. A deployer running this exact system would
be correctly steered to route this stream by difficulty.

\begin{table}[t]
\centering
\caption{Live online run, $300$-task FinQA$+$MATH stream, decisions
made sequentially under a real-time paced budget. Realized
dollar-weighted loss (DWL), premium-tier utilization, unweighted
accuracy. The closed-loop \textsc{seq} policy matches the uniform
accuracy of the one-shot routers while spending a fifth of their
premium budget.}
\label{tab:online}
\small
\begin{tabular}{lrrrl}
...
\textsc{seq} (verifier-gated) & $21{,}627$ & $+11.5\%$ & $\mathbf{5.0\%}$ & $70.0\%$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{The closed loop conserves premium correctly.} The
verifier-gated \textsc{seq} policy is the operational payoff. Its
gold-free verifier scored the cheap-tier attempts confidently
(mean $9.3/10$) and triggered escalation on only $15/300$ tasks, so
\textsc{seq} shipped at $5.0\%$ premium utilization --- against
$19$--$25\%$ for the one-shot routers --- while \emph{matching}
their unweighted accuracy ($70.0\%$, identical to random,
within $0.7$ points of difficulty). On a stream where the premium
tier buys little average accuracy, the live closed loop correctly
declines to spend it, in flight, with no oracle. This is the
online-operation analogue of the offline sequential result above
(Appendix~\ref{app:within}, ``removing the oracle verifier''), now
demonstrated with real-time arrivals and live inference rather than
replayed from a cache.

\paragraph{Scope.} This run demonstrates that the pipeline operates
online and behaves as the framework predicts; it is not a live
\emph{consequence-wins} demonstration, because a live stream in the
software-engineering regime where consequence is the correct signal
would require executing generated patches against test suites at
each arrival --- outside what a lightweight probe can run. The
consequence-wins case under sequential, no-oracle decisions is
established on cached SWE-bench traces in
Appendix~\ref{app:within}; the present run establishes that the same
decision machinery runs live and honors the boundary conditions when
the stream falls in difficulty's regime. A production online system
under real traffic remains the natural next step.
