...
further.} We show this with two complementary experiments: a large-scale
$16$-model SWE-bench capability tier, and a controlled experiment that
holds the model fixed and varies only best-of-$k$ attempts, isolating
compute from model identity (\S\ref{sec:withinmain}). Under matched
compute, the Qwen issue-only consequence predictor reduces
cost-weighted loss by $21.8\%$ relative to difficulty-aware routing and
recovers nearly all of the consequence-oracle gain; the controlled
experiment reproduces the same ordering with bootstrap intervals
bounded away from zero under two judge families. A priority-aware
...
without access to gold patches or benchmark outcome statistics. We
stress that the consequence gain is a \emph{cost-weighted}, not an
accuracy, effect: the raw resolve rate of consequence-aware routing
($0.184$) is within noise of random ($0.180$; Table~\ref{tab:main},
last column), since both send the same number of tasks to the premium
tier --- what changes is \emph{which} tasks, and the cost-weighted
objective is what rewards routing the expensive ones there. Reporting
accuracy alone would hide the entire effect, which is exactly the
point of the paper. For
