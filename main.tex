...
experiments that trade off realism against control. The first
(\S\ref{sec:exp}) routes tasks across a $16$-model SWE-bench
...
($\rho(\mathrm{consequence}, \mathrm{easy}) = -0.12$, n.s.), an
\emph{independent, non-LLM} confirmation of the paper's central
claim that consequence is not a relabeling of difficulty
...
wins ($-32.7\%$ realized dollar-weighted loss vs.\ random),
consequence-aware routing beats random ($-18.8\%$; paired bootstrap
...
\begin{tabular}{lrrrl}
\toprule
Policy & DWL & vs.\ random & premium & accuracy \\
\midrule
...
$23$--$25\%$ for the one-shot routers --- while \emph{matching}
