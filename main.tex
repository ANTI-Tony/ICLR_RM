...
principle: \emph{spend more compute on harder tasks}.
...
Motivated by this gap, we propose \emph{consequence-aware test-time
...
\textbf{Definition.} We define the \emph{consequence} of a task as an ordinal label in $\{0, 1, 2\}$ reflecting the realized deployment cost if the fix is wrong:
...
(i) A \emph{rule-based} labeler using lexical patterns in file paths and gold patches (e.g., \texttt{delete}, \texttt{auth}, \texttt{migration}).
(ii) An \emph{LLM-with-patch judge} (Qwen3-8B) that observes issue text, gold patch, and modified files. This serves as the \textbf{primary} consequence label in our experiments because it can see the realized fix.
(iii) An \emph{LLM-issue-only predictor} (\S\ref{sec:pred}) that sees only the issue text and file path, matching the information available to a deployment-time scheduler. It tests whether consequence can be predicted \emph{before} solving the task.
...
\textbf{Qwen3-8B (hybrid thinking).} Qwen3-8B~\citep{qwen2025qwen3} alternates
...
\textbf{Claude Sonnet 4.5 with extended thinking.} We evaluate Claude~\citep{anthropic2025claude45}
...
\midrule
Qwen3-8B issue-only \emph{(primary)}
...
count and form two tiers: a \emph{cheap} tier consisting of the bottom
4 models, and a \emph{premium} tier consisting of the top 4 models.
...
robustness check. \emph{Consequence-aware (oracle)} replaces the
predictor with the LLM-with-patch label, isolating the upper bound of
a one-signal consequence router. Finally, \emph{Priority-aware
...
single reasoning model. A complementary evaluation would vary the
thinking-token budget of one fixed frontier model under the same
allocation rule. Another complementary direction is to evaluate
domains where high-consequence errors are more frequent, such as
database operations, schema migrations, permission checks, and data
consistency bugs. These settings would test the same allocation
principle under more controlled or more risk-heavy deployment
conditions.
