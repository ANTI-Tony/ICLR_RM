...
(Appendix~\ref{app:seconddomain}), and every qualitative conclusion
survives replacing the labeling judge with a different model family
...
scheduler must adopt \emph{some} error-cost weighting; uniform
...
text-to-SQL) --- with a fully end-to-end pipeline (live two-tier
...
(\texttt{deepseek-chat}), applying the byte-identical with-patch
...
GPT-4o-mini and the premium tier is o4-mini, three independent
...
every medical, toxicological, and financial query). The two judges
agree at only $\kappa = 0.133$. In code, where the rubric was
