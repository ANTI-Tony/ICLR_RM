...
\maketitle
\begin{abstract}
Modern reasoning models allocate test-time compute --- thinking tokens, model calls, or attempts --- by predicted difficulty, implicitly assuming that every failure costs the same. In deployment it does not: a typo in a log message and a migration that corrupts a production database are both one benchmark failure but carry very different costs. We propose consequence-aware compute allocation: a lightweight predictor estimates from the issue text how costly an error would be \emph{before} the task is solved, and the scheduler routes higher-consequence tasks to more compute under the same total budget.
On a $16$-model SWE-bench Lite compute tier, the deployment-time consequence router reduces cost-weighted loss by $21.8\%$ over difficulty-aware routing, and a priority variant that additionally uses marginal-gain information reaches $30.7\%$ (an analysis upper bound); because the hardest tasks are often unsolvable at any tier, difficulty-aware routing itself does \emph{worse than random}. The predictor never misclassifies a high-consequence task as low-consequence ($0/110$), and a controlled experiment that holds the model fixed and varies only best-of-$k$ attempts reproduces the same ordering. We test generalization on $1{,}290$ tasks across six pools --- a second database/ORM pool from SWE-bench Verified, cross-dataset transfer to Multi-SWE-bench mini, and end-to-end out-of-domain probes on BIRD text-to-SQL, FinQA, and MATH --- where the framework's own boundary conditions hold: consequence-aware routing wins exactly when error costs are heterogeneous, and its edge vanishes precisely where predicted (homogeneous costs, no premium headroom, or a positive difficulty--gain coupling, where difficulty rightly wins).
\end{abstract}
